...
// ../.wrangler/tmp/bundle-IHsfd2/checked-fetch.js
...
// ../.wrangler/tmp/bundle-IHsfd2/strip-cf-connecting-ip-header.js
...
async function onRequestGet(context) {
  ...
  if (!apiUrl)
    return new Response("API_URL not configured", { status: 500, headers: corsHeaders });
...
async function onRequestGet2(context) {
  ...
  let apiUrl = env.API_URL;
  if (!apiUrl)
    return new Response(JSON.stringify({ error: "API_URL not configured" }), { status: 500, headers: corsHeaders });
  if (!apiUrl.startsWith("http"))
    apiUrl = `https://${apiUrl}`;
  try {
    const url = new URL(request.url);
...
// ../.wrangler/tmp/pages-VghPP1/functionsRoutes-0.059510941260643824.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-IHsfd2/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-IHsfd2/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.48553871960814354.mjs.map
