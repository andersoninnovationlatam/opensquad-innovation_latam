...
input:
  ...
  - squads/agent-social-v3/pipeline/data/visual-identity.md
...
output:
  - squads/agent-social-v3/output/slides/slide-01.html
  - squads/agent-social-v3/output/slides/slide-02.html
  - squads/agent-social-v3/output/slides/slide-NN.html
  - squads/agent-social-v3/output/slides/design-documentation.md
---
...
1. **Orquestrador (recomendado):**  
   `node squads/agent-social-v3/scripts/run-squad-design.mjs {run_id}`  
...
4b. **Regra Crítica de Contraste (Overlay Diferenciado):** 
Para garantir legibilidade perfeita (WCAG AA) sobre imagens fotográficas imprevisíveis, você DEVE aplicar a seguinte regra de CSS para o overlay:

- **Slide 1 (Hook):** Usar overlay em gradiente para criar respiração visual no topo da imagem:
  `background: linear-gradient(to bottom, rgba(21, 10, 28, 0.3) 0%, rgba(21, 10, 28, 0.75) 50%, rgba(21, 10, 28, 0.95) 100%);`

- **Slides 2 a 6:** Usar overlay sólido mais escuro para garantir máxima legibilidade do texto branco em qualquer fundo:
  `background: rgba(21, 10, 28, 0.88);`

É PROIBIDO usar `rgba(21,10,28,0.85)` nos slides 2-6, pois essa opacidade falha em imagens com fundos muito claros. Use sempre `0.88`.


5. **Converter o logo a data-URI.** Ler `squads/agent-social-v3/assets/innovation-latam-logo-white.png` e reutilizar em todos os slides. Posição obrigatória: **`position:absolute; top:30px; left:100px`** (única norma — ver `real-imagery-and-safe-zones.md` §0). Nunca usar coordenadas diferentes sem nota em `design-documentation.md`.
...
8. **Criar slide-01.html (Cover Layout).** Antes de escrever qualquer HTML: **transcrever verbatim do `carousel-draft.md` o texto do Slide 1** (headline e body), colocando-o em um comentário HTML imediatamente antes do bloco de conteúdo (`<!-- DRAFT: ... -->`). O HTML deve conter **somente** as palavras presentes nesse trecho — nenhuma palavra de outro slide ou gerada por inferência. Hook na banda **810px + 270px altura**, **à esquerda**; após inject: foto full-bleed; overlay; **padding 100px** no bloco; **rodapé** `@innovationlatam` + `ARRASTE ->`.

9. **Verificar slide 1 antes de avançar.** Confrontar o texto visível no HTML com o comentário `<!-- DRAFT: ... -->` palavra por palavra. Logo, WCAG AA, sem overflow, **1080×1350px**.
...
11. **Criar slides 03+.** Para cada slide: **transcrever verbatim o texto do `carousel-draft.md`** para o comentário `<!-- DRAFT: ... -->` antes do bloco de conteúdo; garantir que o HTML contém somente essas palavras; garantir **fundo IA** distinto; layout Standard/Split com texto **à esquerda**, **padding 100px**. WCAG AA por slide.
