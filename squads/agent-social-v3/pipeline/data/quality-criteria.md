# Quality Criteria — Social Media Innovation Latam v2
...
- **CONDITIONAL APPROVE**: Média ≥ 7/10 com itens não-bloqueantes identificados
- **REJECT**: Média < 7/10 OU qualquer critério abaixo de 4/10 OU veto ativado
- **Critério com peso 1.5×**: C1 Scroll-Stop Test (critério prioritário — calibrado como mais importante)
...
### C4: Reflexão Final
**Pergunta:** O penúltimo slide entrega uma reflexão memorável (síntese emocional, não dado novo)?
...
- 9-10: CTA específico e inteligente com duas ações (salvar + comentar) conectadas ao tema.
...
**Pergunta:** O copy está no tom aprovado pela Innovation Latam (didático-revelador, dados reais, energia empreendedora)?
- 9-10: Tom perfeitamente alinhado. Lê exatamente como Innovation Latam deve soar.
- 7-8: Majoritariamente alinhado. Um ou dois momentos poderiam ser mais "Innovation Latam".
- 5-6: Tom correto mas escrita genérica. Poderia ser de qualquer marca de inovação.
...
**Pergunta:** Todos os slides usam o sistema de design post-05 (Montserrat, paleta roxa, logo) e a norma do squad (texto à esquerda; rodapé `@innovationlatam` + `ARRASTE ->`)?
- 9-10: Sistema post-05 perfeitamente consistente. Qualquer slide reconhecível como Innovation Latam.
- 7-8: Consistência alta com uma ou duas variações aceitáveis dentro dos tokens.
- 5-6: Maioria consistente mas alguns slides "fogem" do sistema post-05.
- 3-4: Inconsistências visíveis. Fontes ou cores fora do sistema sem justificativa.
- 1-2: Sem sistema de design. Cada slide parece de um carrossel diferente.

### V2: Readability
**Pergunta:** O texto é legível em tela de celular com os padrões do sistema post-05?
- 9-10: Body ≥ 36px (conforme post-05), hero ≥ 56px. Contraste WCAG AA (4.5:1) em todos os slides.
...
### V3: Visual Impact (Padrão post-05 roxa/teal)
**Pergunta:** O visual é impactante e fiel à identidade roxa/teal da Innovation Latam?
- 9-10: Visual que para o scroll. Alto contraste, paleta roxa/teal reconhecível, slides memoráveis que pertencem à Innovation Latam.
- 7-8: Visual forte e coerente com o padrão post-05. Logo bem posicionado; rodapé `@innovationlatam` + `ARRASTE ->` visível.
- 5-6: Visual mediano. Identidade presente mas sem impacto.
- 3-4: Visual fraco. Cores fora do padrão, logo ausente ou posicionado incorretamente.
- 1-2: Visual que ativamente prejudica o conteúdo ou não tem relação com a identidade Innovation Latam.
...
### T1: Image Specs
- [ ] Formato: JPEG em `rendered/` (gerado via Playwright / `inject-bg-and-render.mjs`)
- [ ] Dimensões: 1080px × 1350px (4:5, obrigatório)
- [ ] Tamanho máximo: 30MB por arquivo
- [ ] Número de slides: **6** (fixo)

### T2: Caption
- [ ] Comprimento: máximo 2.200 caracteres
- [ ] Hashtags: 8-12 ao final
...
3. **Total de slides diferente de 6** — fora da especificação do squad v2
...
7. **Fonte diferente de Montserrat em qualquer slide** — violação do sistema de design post-05
8. **Logo Innovation Latam ausente em qualquer slide** — violação de identidade de marca
9. **Contraste abaixo de 4.5:1 em texto sobre imagem sem overlay forte** — falha WCAG AA
