...
## Extensões obrigatórias — agent-social-media-v2

Este squad **estende** o post-05 com:

1. **`pipeline/data/real-imagery-and-safe-zones.md`** — viewport **1080×1350px (4:5)**; fundo fotográfico por slide (pipeline `generate-bg-image` + `inject-bg-and-render`); **safe zone** inferior (últimos 200px); **todos os slides** texto editorial **à esquerda**; **padding lateral 100px**; **rodapé** `@innovationlatam` + `ARRASTE ->` em todos os slides; tipografia legível em mobile; export **JPEG** em `rendered/`.
2. **`pipeline/data/layout-variations.md`** — estruturas HTML/CSS (capa banda 60%–80%; slides 2–6 à esquerda; padding 100px; rodapé obrigatório).

**Ordem de precedência:** `real-imagery-and-safe-zones.md` > secções genéricas abaixo que contradizem esse brief.
...
- **Texto (squad v2):** copy **curto** — ver `carlos-conteudo` / `real-imagery-and-safe-zones.md`; não usar meta de 60 palavras no slide.
- **Tipografia:** **Montserrat** em todo o slide; headline **700 / 46px**; body **500 / 36px**; destaques **`#c0fefd`** (teal).
- **Conteúdo:** slide de contexto com 2–3 frases curtas; detalhe na **caption**.
- **Layout:** headline + body; texto **à esquerda** na área útil; **padding** alinhado à norma (**100px** laterais no squad atual).
- **Rodapé no canvas:** `@innovationlatam` (esquerda) + `ARRASTE ->` (direita) — ver `real-imagery-and-safe-zones.md`.

### Slides 3+ — Conteúdo (referência: `slide-conteudo-01`)

- **Fundo:** roxo placeholder ou foto IA após inject (norma atual).
- **Tipografia:** **Montserrat** em todo o slide; body **500 / 40px**; parágrafos com espaçamento entre blocos (~`1.35em`); destaques **`#c0fefd`**.
- **Layout:** texto **à esquerda** na área útil; **padding lateral 100px** (squad atual).
- **Rodapé no canvas:** `@innovationlatam` + `ARRASTE ->` (obrigatório).
...
- **Pesos utilizados:**
  ...
  - Rodapé com handle no slide: **não** usar no squad v2 (identidade na caption/perfil)
- **Não usar** Bebas Neue, Playfair Display, Inter ou qualquer outra fonte.

### Logo e automação

- Logo branco **sempre** no HTML gerado pelo build (data-URI de `assets/innovation-latam-logo-white.png`), canto superior esquerdo, `max-width: 200px`, margem **30px** — presente em **todos** os renders após Playwright.
...
**Viewport (este squad):** 1080px × **1350px** (Instagram feed **4:5**)
...
- Left: Brand / handle — no squad v2: `@innovationlatam` (genérico post-05: texto "Innovation Latam", 24px / 600 weight / gray `#888888`)
- Right: Swipe indicator `ARRASTE ->` (24px / muted gray `#666666`; no squad v2 usar texto ASCII conforme norma)
...
**Logo/Brand Presence:**
- Text-based: squad v2 — `@innovationlatam` no rodapé; genérico post-05: "Innovation Latam" no footer
- Style: 24px / 600 weight / gray (#888888)
- Position: Bottom left of every slide

**Swipe Indicator:**
- Text: `ARRASTE ->` (Portuguese for "Swipe"; no squad v2, norma fixa com seta ASCII)
...
5. **Viewport exact (agent-social-media-v2):** Body width and height must be exactly **1080px × 1350px**
...
**Squad `agent-social-media-v2`:** seguir `layout-variations.md` + `real-imagery-and-safe-zones.md` — **1080×1350px**, **slide 1** hook à esquerda na banda; **slides 2–6** texto **à esquerda**; **padding lateral 100px**; **rodapé** `@innovationlatam` + `ARRASTE ->` em todos os slides.
...
- **Rodapé no slide (squad v2):** `@innovationlatam` + `ARRASTE ->` na faixa inferior (safe zone), conforme `real-imagery-and-safe-zones.md`.
