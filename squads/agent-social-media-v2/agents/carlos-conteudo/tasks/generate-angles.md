...
- Todo texto produzido por Carlos nesta task (`hook_preview`, `justification`, labels descritivos em comentários) em **português brasileiro** com **ortografia e gramática** corretas.
- Cada `hook_preview` e cada `justification` preenchida **começa com letra maiúscula**; frases após ponto final também.

---

## Process

0. **Ler o ângulo escolhido pelo usuário (obrigatório).** No topo de `input/content.md`, o YAML frontmatter deve incluir `angulo_escolhido` com **exatamente um** destes IDs: `medo` | `oportunidade` | `educacional` | `contrario` | `inspiracional`. Se estiver ausente, inválido ou ambíguo, **não** prosseguir: a tarefa está bloqueada até o usuário preencher. (Alternativa aceita: linha isolada `angulo_escolhido: contrario` imediatamente após o primeiro `---` do frontmatter, no mesmo bloco YAML.)

1. **Absorver o texto do usuário.** Ler o corpo de `input/content.md` (abaixo do frontmatter) e identificar o **dado-âncora** (número ou afirmação factual mais forte do texto). O dado-âncora é o ponto de partida para os ângulos.
...
5. **Aplicar a escolha do usuário.** Marcar `selected: true` **somente** no ângulo cujo `id` coincide com `angulo_escolhido` do frontmatter. Todos os outros ficam com `selected: false`. No ângulo selecionado, a `justification` deve: (1) reconhecer que o ângulo foi **definido pelo usuário** em `input/content.md`; (2) em 2-3 linhas, explicar como o dado-âncora e a notícia se prestam a esse ângulo (encaixe editorial, não comparação com outros ângulos).
...
7. **Preencher `angulo_selecionado`** no YAML com o mesmo ID que `angulo_escolhido` do input (devem ser idênticos).

## Output Format

```yaml
# angles.yaml — gerado por Carlos Conteúdo
tema: "[TEMA DO TEXTO / NOTÍCIA]"
dado_ancora: "[DADO ÂNCORA IDENTIFICADO EM input/content.md]"
fonte_ancora: "[FONTE NO PRÓPRIO TEXTO, ex.: veículo citado ou 'texto fornecido']"
...
angulos:
  ...
  - id: contrario
    ...
    score_engajamento: "[1-5]"
    selected: true
    justification: "[2-3 linhas: ângulo definido pelo usuário + como dado-âncora e notícia honram este ângulo]"
...
angulos:
  ...
  - id: contrario
    ...
    score_engajamento: 5
    selected: true
    justification: "Ângulo definido pelo usuário em input/content.md (angulo_escolhido: contrario). O dado-âncora 78% vs 12% é intrinsecamente contraintuitivo; a narrativa Contrário faz o gap declaração/execução explodir no feed. Para a audiência Innovation Latam, isso espelha o dia a dia de quem vê slide de estratégia de IA e zero piloto em produção."
...
- Exatamente 1 ângulo com `selected: true` — o que coincide com `angulo_escolhido` do usuário; justificativa de encaixe (2-3 linhas)
- Scores preenchidos para todos os 5 ângulos nos 3 critérios
- `angulo_selecionado` no YAML = `angulo_escolhido` do input
- Output é YAML válido e parseável

## Veto Conditions

1. `angulo_escolhido` ausente ou ID inválido em `input/content.md` — tarefa bloqueada até correção
2. Menos de 5 ângulos gerados — tarefa incompleta
3. Mais de 1 ângulo marcado como `selected: true`, ou o selecionado ≠ `angulo_escolhido` do usuário — ambiguidade que bloqueia o próximo step
