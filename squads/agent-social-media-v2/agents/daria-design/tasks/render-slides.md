...
order: 2
input:
  - squads/agent-social-media-v2/output/slides/v1/slide-01.html
  - squads/agent-social-media-v2/output/slides/v1/slide-NN.html
output:
  - squads/agent-social-media-v2/output/slides/v1/rendered/slide-01.jpg
  - squads/agent-social-media-v2/output/slides/v1/rendered/slide-NN.jpg
---
...
2. **Se os JPEGs ainda não existem:** **executar** (não adiar) para cada `NN` de `01` a `06` (a partir da raiz do repo). Se faltar `backgrounds/bg-NN.*`, **sempre** gerar antes do inject:
   ```bash
   ...
   ```
   **Ou** uma vez: `node squads/agent-social-media-v2/scripts/run-squad-design.mjs {run_id}` (regenera também HTML via `gen-slides-for-run.mjs`; usar só se aceitável).
...
- N HTMLs = N JPEGs em `rendered/`
- Dimensões 1080×1350px; logo e copy conforme norma

## Veto Conditions

1. Contagem de JPEGs ≠ número de HTMLs
