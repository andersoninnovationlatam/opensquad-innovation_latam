 ...
 *
 * Uso (raiz do repo):
 *   node squads/agent-social-media-v2/scripts/run-squad-design.mjs <run-id>
 *
...
const slidesDir = path.join(REPO_ROOT, "squads/agent-social-media-v2/output", runId, "slides");
...
runNode("squads/agent-social-media-v2/scripts/gen-slides-for-run.mjs", [runId]);

const slides = ["01", "02", "03", "04", "05", "06"];
for (const n of slides) {
  console.log(`\n── Slide ${n}: generate-bg-image ──`);
  runNode("squads/agent-social-media-v2/scripts/generate-bg-image.mjs", [n, slidesDir]);
  console.log(`── Slide ${n}: inject-bg-and-render ──`);
  runNode("squads/agent-social-media-v2/scripts/inject-bg-and-render.mjs", [n, slidesDir]);
}
