# Domain Framework — Social Media Innovation Latam
...
O pipeline transforma um texto/notícia em input/ em slides PNG renderizados via Playwright, prontos para publicação no feed Instagram.
...
**Quem adiciona a notícia em `input/content.md` também escolhe o ângulo** (frontmatter `angulo_escolhido`: `medo` | `oportunidade` | `educacional` | `contrario` | `inspiracional`). Carlos Conteúdo **não substitui** essa escolha: documenta os cinco ângulos em `angles.yaml` para referência, marca `selected` apenas no ID escolhido pelo usuário e gera o copy dos slides nessa lente.

Critérios úteis para o **usuário** ao escolher o ângulo:
1. **Match com o dado-âncora**: qual lente faz o gancho mais forte no feed?
...
4. **Evitar saturação**: variar o ângulo entre posts consecutivos
...
- **Máximo:** 9 slides
- **Padrão:** 8 slides
- Nunca menos de 7, nunca mais de 9
...
| Slide 2 | Contexto | Por que esse dado importa? O que ele revela? Maior carga de texto do carrossel. Imagem full-bleed com overlay. |
| Slides 3-N-2 | Desenvolvimento | 1 ideia por slide. Máx 4-5 linhas visíveis. Cada slide avança a narrativa. Sem repetição. |
| Slide N-1 | Reflexão | Síntese emocional. Pergunta que incomoda construtivamente. NÃO um dado novo. Tratamento visual distinto. |
| Slide N | CTA | 2 ações: salvar + comentar. Conectado especificamente ao tema. Tratamento visual distinto. |
...
- Logo: branco, canto superior esquerdo, `max-width: 200px`, margem `30px`
- Rodapé: `@innovationlatam` esquerda | `ARRASTE →` direita
...
1. **Cover Layout** (slide 1) — hero text terço inferior, fundo imagem full-bleed
2. **Image Context Layout** (slide 2) — imagem full-bleed + overlay forte + texto maior
3. **Standard Content Layout** (slides 3+) — roxo sólido, texto alinhado à esquerda
4. **Split Content Layout** (variação slides 3+) — headline topo + callout box roxo
5. **Reflection Layout** (penúltimo) — espaçamento maior, texto centralizado, gradiente sutil
6. **CTA Layout** (último) — high contrast, elemento tipo botão em teal ou gold

### Geração HTML

- HTML auto-suficiente por slide: CSS inline, @import Montserrat, body 1080×1440px
...
input/content.md
      ↓
  [step-01] Rafael Rastreio
  Pesquisa e enriquecimento com dados verificados
      ↓
  output/research.md
      ↓
  ...
  output/angles.yaml
      ↓
  [step-03] Carlos Conteúdo
  Copy completo de todos os slides + caption + hashtags
      ↓
  output/carousel-draft.md
      ↓
  [step-04] Dária Design
  HTMLs auto-suficientes por slide (padrão post-05)
      ↓
  output/slides/slide-NN.html
      ↓
  [step-05] Dária Design
  Renderização Playwright → PNG
      ↓
  output/slides/rendered/slide-NN.png
      ↓
  [step-06] Beatriz Balizadora
  Revisão C1-C6 + V1-V3, scoring, veredicto
      ↓
  output/review.md
```
...
Se Beatriz emite REJECT, o pipeline retorna ao step-03 (create-slides) para reescrita do copy. Carlos recebe o feedback da revisão como input adicional.

---

## Parte 5: Sistema de Revisão de Qualidade
...
3. Menos de 7 slides ou mais de 9 slides
