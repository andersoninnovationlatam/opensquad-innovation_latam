...
inputFile: squads/agent-social-media-innovation-latam/output/research.md
outputFile: squads/agent-social-media-innovation-latam/output/angles.yaml
---

## Context Loading

Antes de executar, carregar:
- `squads/agent-social-media-innovation-latam/input/content.md` — notícia do usuário + **angulo_escolhido** obrigatório no frontmatter YAML
- `squads/agent-social-media-innovation-latam/output/research.md` — brief de pesquisa do Rafael
- `squads/agent-social-media-innovation-latam/pipeline/data/domain-framework.md` — os 5 ângulos e critérios de seleção
- `squads/agent-social-media-innovation-latam/pipeline/data/tone-of-voice.md` — os 6 tons e vocabulário
- `squads/agent-social-media-innovation-latam/pipeline/data/anti-patterns.md` — o que evitar no copy
- `squads/agent-social-media-innovation-latam/agents/carlos-conteudo/tasks/generate-angles.md` — task file detalhado

## Instructions

### Process

0. **Validar o ângulo do usuário.** Ler o frontmatter de `input/content.md` e confirmar `angulo_escolhido` (`medo` | `oportunidade` | `educacional` | `contrario` | `inspiracional`). Se ausente ou inválido, parar até correção.

1. **Absorver o brief completo de Rafael.** Ler `output/research.md` com foco especial no [ANCHOR DATA] — o dado-âncora é o ponto de partida obrigatório para todos os ângulos.

2. **Identificar a tensão central dos dados.** Qual é o conflito ou revelação que emerge dos dados? (ex: "declaram vs. executam", "sabem vs. fazem", "querem vs. conseguem").
...
5. **Aplicar a escolha do usuário.** Marcar `selected: true` **somente** no ângulo cujo `id` = `angulo_escolhido`. Incluir `angulo_escolhido_pelo_usuario` no YAML. Justificativa: ângulo definido pelo usuário + encaixe com o brief (2-3 linhas). Não substituir o ângulo do usuário.
...
fonte_ancora: "[FONTE]"
angulo_escolhido_pelo_usuario: "[id do frontmatter]"
...
angulos:
  ...
  - id: oportunidade
    ...
    score_engajamento: 4
    selected: true
    justification: "Ângulo definido pelo usuário em input/content.md (angulo_escolhido: oportunidade). O ROI 3.2× pede narrativa de janela e retorno; o dado dos 71% que desistem antes de 6 meses fecha o gancho com urgência sem virar medo puro."
...
- Justificativa no ângulo selecionado: encaixe editorial (usuário já escolheu a lente)
